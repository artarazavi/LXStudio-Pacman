...
@LXCategory("Arta")
public class PacmanV2HardPattern extends GLShaderPattern {
  ...
  public PacmanV2HardPattern(LX lx) {
    ...
    addShader(
        GLShader.config(lx)
            .withFilename("pacman_v2_hard.fs")
  ...
  private float computeMouthOpen() {
    ...
    float wave =
        (float) ((Math.sin(getTime() * this.mouthSpeed.getValue() * Math.PI * 2) + 1) * 0.5);
